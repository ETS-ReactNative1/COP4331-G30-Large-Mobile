...
export default class WaterAddButtonComponent extends Component {
  render () {
    return (
  ...
  addLogClick = async (props) =>
  {
    ...
    props.onAmountValidChange(true);
    props.onMessageChange('');

    var dateReg = new RegExp('(0[1-9]|1[0-2])/(0[1-9]|[12][0-9]|3[01])/[0-9]{4}');
    var amountReg = new RegExp('([0-9]+.[0-9]+)|([0-9]+)');
    ...
    // Verify FL OZ amount format
    if (!amountReg.test(props.amount))
    {
      props.onMessageChange("Amount must be a number greater than 0");
      props.onAmountValidChange(false);
      return;
    }

    try
    {
      const newAmount = Number(props.amount.trim());
      ...
      const endpoint = 'https://cop4331-g30-large.herokuapp.com/api/water/' + global.username.trim();
