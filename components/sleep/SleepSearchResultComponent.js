...
export default class SleepSearchResultComponent extends Component {
  ...
  render() {
    ...
    !this.state.isLogDeleted &&
      <View style={styles.container}>
        <View style={styles.sleepHabit_Item}>
          <Text style={styles.sleepHabit_Date}>{this.props.date}</Text>
          <Text style={styles.sleepHabit_Amount}>Hours Slept: {this.props.hours} Hours</Text>
          <TouchableOpacity 
            style={styles.sleepHabit_RemoveButton}
            onPress={() => this.deleteLogClick(this.props)}
          >
            <FontAwesomeIcon
                name="trash"
                style={styles.sleepHabit_RemoveIcon}
            ></FontAwesomeIcon>
          </TouchableOpacity>
        </View>
      </View>
  ...
  removeLog = async (props) =>
  {
    try
    {
      ...
      const endpoint = 'https://cop4331-g30-large.herokuapp.com/api/deleteSleep/' + global.username.trim();
  ...
  deleteLogClick = async (props) =>
  {
    Alert.alert(
      "Delete Log",
      "Are you sure you want to delete you \"Sleep\" log for " + props.date + "?",
      [
        { text: "Yes", 
